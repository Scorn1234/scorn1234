...
def send_miss_you(request, contact_id, message):
    ...
    message = "계약자 '{0}'님이 아이가 잘 있는지 궁금해 합니다. 사진을 보내주세요!".format(get_user(request))
...
def create_contact(form, post):
    if form.is_valid():
        ...
        send_message(contact.sitter.phoneNumber, "새로운 계약 요청이 들어왔습니다. 지금 확인해보세요.")
        return HttpResponseRedirect("/contact")
...
def reviews(request, contact_id=1, grade=1):
    form = ReviewForm(request.POST or None, request.FILES or None)
    if request.method == 'POST':
        if form.is_valid():
            p
            contact = Contact.objects.get(int(contact_id))
            ...
            contact.save()
            Review.save()
            return HttpResponseRedirect('/contact/review')
        else:
